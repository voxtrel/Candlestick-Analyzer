...
{
    /// <summary>
    /// This is the recognizer class for the Bullish Engulfing pattern, which uses the recognizer abstract class as a template
    /// </summary>
    internal class Recognizer_BullishEngulfing : Recognizer
    {
        ...
        public override bool Recognize(List<SmartCandlestick> lscs, int currIndex)
        {
            ...
            else
            {
                ...
                currC.CandleProperties.Add(patternName, isPrevBearish && isCurrBullish && isPrevBodyContained);     // Add the result to the dictionary

                // Check if the previous candlestick is bearish and the current candlestick is bullish with engulfing body
                return isPrevBearish && isCurrBullish && isPrevBodyContained;
            }
