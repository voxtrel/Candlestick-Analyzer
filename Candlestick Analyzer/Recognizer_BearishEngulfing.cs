...
{
    /// <summary>
    /// This is the recognizer class for the Bearish Engulfing pattern, which uses the recognizer abstract class as a template
    /// </summary>
    internal class Recognizer_BearishEngulfing : Recognizer
    {
        ...
        public override bool Recognize(List<SmartCandlestick> lscs, int currIndex)
        {
            ...
            {
                bool isPrevBullish = prevC.close > prevC.open;                                       // Check if previous candlestick is bullish
                bool isCurrBearish = currC.close < currC.open;                                       // Check if current candlestick is bearish
                bool isPrevBodyContained = (currC.close < prevC.open) && (currC.open > prevC.close); // Check if the body of current candlestick engulfs the previous candlestick

                currC.CandleProperties.Add(patternName, isPrevBullish && isCurrBearish && isPrevBodyContained);     // Add the result to the dictionary
                // Check if the previous candlestick is bullish and the current candlestick is bearish with engulfing body
                return isPrevBullish && isCurrBearish && isPrevBodyContained;
            }
