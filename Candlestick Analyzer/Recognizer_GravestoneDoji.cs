...
{
    /// <summary>
    /// This is the recognizer class for the Gravestone Doji pattern, which uses the recognizer abstract class as a template
    /// </summary>
    internal class Recognizer_GravestoneDoji : Recognizer
    {
        ...
        public override bool Recognize(List<SmartCandlestick> lscs, int currIndex)
        {
            ...
            {
                bool cv = ((cs.BodyRange <= (decimal)0.15 * cs.Range) && (cs.TopTail > (decimal)0.75 * cs.Range));  // Else, calculate the pattern
