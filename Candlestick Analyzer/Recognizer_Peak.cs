...
{
    /// <summary>
    /// This is the recognizer class for the Peak pattern, which uses the recognizer abstract class as a template
    /// </summary>
    internal class Recognizer_Peak : Recognizer
    {
        ...
        public override bool Recognize(List<SmartCandlestick> lscs, int currIndex)
        {
            ...
            {
                bool isPrevCHighest = prevC.high > prevPrevC.high && prevC.high > currC.high;   // Evaluate the pattern
                currC.CandleProperties.Add(patternName, isPrevCHighest);                        // Add the result to the dictionary
                return isPrevCHighest;                                                          // Return result
            }
